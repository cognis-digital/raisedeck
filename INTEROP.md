...
How **raisedeck** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `raisedeck` lives in the **Maritime domain awareness** cluster.
...
| from | relation | to |
|---|---|---|
| `raisedeck` | JSON findings compose with | [`maritimeint`](https://github.com/cognis-digital/maritimeint), [`uaslog`](https://github.com/cognis-digital/uaslog), [`corpmap`](https://github.com/cognis-digital/corpmap) |
| `raisedeck` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `raisedeck` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `raisedeck` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the maritime domain awareness cluster.** `raisedeck` output feeds the next tool:
```bash
raisedeck ... --format json > out.json          # this tool's findings (see `raisedeck --help`)
...
raisedeck ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
raisedeck ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
raisedeck ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `raisedeck`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `raisedeck` slots
